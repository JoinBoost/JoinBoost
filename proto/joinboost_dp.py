...
class joinGraph:
    ...
    def create_dummy_model(self,replace=False):
        ...
        if self.fact_pandas:
            ...
            self.fact_table["s"] = np.floor(self.fact_table["s"]).astype("int64")
            elapsed_time = time.time() - start_time
            self.totaltime += elapsed_time
...
#         self.execute_query(query)
    ...
    def clean_leaves(self):
        ...
        for i in range(len(self.leaves.queue)):
            ...
            pre = math.floor(cjt.ts/cjt.tc * self.learning_rate)
            if len(ann) == 0:
...
ADD %(column)s BIGINT NOT NULL CONSTRAINT %(constraint_name)s  DEFAULT 1;'''% { 'table_name': table_name, 'column': column, 'constraint_name': constraint_name}
    ...
    def downward_message_passing(self, root, cjt, sql_only = False):
        if self.log:
            print("--downward message passing for cjt " + str(cjt.id))
        
        if sql_only:
            pairs = []
            def pre_dfs(table_name, parent, pairs, from_fact):
                ...
                for child in self.joins[table_name]:
                    if parent is None or child != parent:
                        pairs.append(self.send_message(table_name, child, cjt, from_fact=from_fact, pandas_as_view=True))
                        pre_dfs(child, table_name, pairs, from_fact)
            pre_dfs(root, None, pairs, False)
            return pairs
        ...
        if root != self.fact:
            ...
            def pre_dfs(table_name, parent, sql, from_fact):
                ...
                for child in self.joins[table_name]:
                    if parent is None or child != parent:
                        query = self.send_message(table_name, child, cjt, from_fact=from_fact, pandas_as_view=True)
                        self.execute_query(query)
            ...
            view_sql_pairs = []
            for child in self.joins[self.fact]:
                pairs = []
                pairs.append(self.send_message(self.fact, child, cjt, from_fact=True, pandas_as_view=True))
                pre_dfs(child, self.fact, pairs, False)
                view_sql_pairs.append(pairs)
                
            self.parallel_message(view_sql_pairs)
...
#             if left_cjt.depth < self.max_depth:
            # we still need to pass messages for fact table semi-join
            
            if self.fact in self.joins:
                view_sql_pairs = []
                pairs = self.downward_message_passing(table, left_cjt, sql_only = True)
                view_sql_pairs.append(pairs)

                pairs = self.downward_message_passing(table, right_cjt, sql_only = True)
                view_sql_pairs.append(pairs)
                self.parallel_message(view_sql_pairs)
    ...
    def parallel_message(self, view_sql_pairs):
        if self.log:
            print(view_sql_pairs)
        ...
        def compute_message(worker_id):
            try:
                ...
                for pair in pairs:
                    viewname, sql = pair
                    globals()[viewname] = con.execute(sql).df()
            except Exception as e: print(e)

        start_time = time.time()
        function_threading(compute_message, len(view_sql_pairs))
        elapsed_time = time.time() - start_time
        self.totaltime += elapsed_time
        if self.log:
            print("multi-thread takes:" + str(elapsed_time))       
        ...
        if self.feature_parallel:
            ...
            for table in self.features:
                for i in range(len(self.features[table])):
                    ...
                    queries.append((table, feature, feature_type,query))
            ...
            def compute_split(worker_id):
                try:
                    ...
                    con.execute(query)
                    splits.append((table, feature, feature_type) + con.fetchall()[0])
                except Exception as e: print(e)

            start_time = time.time()
            function_threading(compute_split, len(queries))
            elapsed_time = time.time() - start_time
            self.totaltime += elapsed_time
            if self.log:
                print("multi-thread takes:" + str(elapsed_time))  
            ...
            for split in splits:
                table, feature, feature_type, cur_value, cur_reduction_in_variance, s, c = split
                c = int(c)
...
#                     continue
    ...
    def update_model_sql(self):
        ...
        for i in range(len(self.leaves.queue)):
            ...
            prediction = math.floor(cur_cjt.ts/cur_cjt.tc * self.learning_rate)
    ...
    def update_error(self):
        if not self.fact_pandas_join:
            for i in range(len(self.leaves.queue)):
                ...
                prediction = math.floor(cjt.ts/cjt.tc * self.learning_rate)
                table = self.fact
                children = []
                if table in self.joins:
                    for child in self.joins[table]:
        ...
        if self.fact_pandas_join:
            def dfs(cjt, sql, left_dummy = False, right_dummy = False):
                indent = ' '*cjt.depth
                if cjt.left_child is None:
                    pre = cjt.ts/cjt.tc * self.learning_rate
                    sql +=  indent + "CAST(" + str(pre) + " AS INT)\n"
                    return sql
            ...
            sqls = dict()
            sql = "SELECT\n"
            sql = dfs(self.cjts[0], sql, left_dummy=True)
            sql += "AS s FROM " + table + "\n"
            sqls[0] = sql
            sql = "SELECT\n"
            sql = dfs(self.cjts[0], sql, right_dummy=True)
            sql += "AS s FROM " + table + "\n"
            sqls[1] = sql
            if self.log:
                print(sqls[0])
            ...
            def compute_update_series(worker_id):
                try:
                    ...
                    series.append(np_s)
#                     print(str(worker_id) + " Done")
                except Exception as e: print(e)
            start_time = time.time()
            function_threading(compute_update_series, 2)
            
            self.fact_table["s"] = self.fact_table["s"] - series[0] - series[1]
            elapsed_time = time.time() - start_time
            self.totaltime += elapsed_time
    ...
    def send_message(self, table1, table2, cjt, identity = False, from_fact = False, pandas_as_view=False):
        ...
        if pandas_as_view:
            sql += self.aggregation_sql(groupby = groupby, variance=variance, join_tables = join_tables, 
                                    select_conditions = select_conditions, left_join_tables = left_join_tables, 
                                    left_join_conditions = left_join_conditions, into = None, count_as_1 = (table1 == self.fact))
            return (into, sql)
        else:
            sql += self.aggregation_sql(groupby = groupby, variance=variance, join_tables = join_tables, 
                                    select_conditions = select_conditions, left_join_tables = left_join_tables, 
    ...
    def create_sample_fact(self, sample_percent = 10, sample_seed = 1):
        sample_table = self.fact + "_sample_" + str(sample_percent) + "_" + str(sample_seed)
        sql = "CREATE OR REPLACE VIEW " + sample_table + " AS\n"
